...
use RustyLeetCode::strings::medium::longest_substring_without_repeating_chars::{
    ...
    longest_substring_without_repeating_chars_sliding_window_ascii::length_of_longest_substring_sliding_window_ascii,
...
fn bench_small(c: &mut Criterion) {
    c.bench_function("Brute Force - Small", |b| {
        b.iter(|| {
            let _ = length_of_longest_substring_brute_force(black_box(SMALL_INPUT.to_string()));
        })
    });

    c.bench_function("Sliding Window - Small", |b| {
        b.iter(|| {
            let _ = length_of_longest_substring_sliding_window(black_box(SMALL_INPUT.to_string()));
        })
    });

    c.bench_function("ASCII Sliding Window - Small", |b| {
        b.iter(|| {
            let _ = length_of_longest_substring_sliding_window_ascii(black_box(
    ...
    c.bench_function("Linked HashMap Sliding Window - Small", |b| {
        b.iter(|| {
            let _ = length_of_longest_substring_linked_hash_map(black_box(SMALL_INPUT.to_string()));
        })
    });
}
...
fn bench_medium(c: &mut Criterion) {
    c.bench_function("Brute Force - Medium", |b| {
        b.iter(|| {
            let _ = length_of_longest_substring_brute_force(black_box(MEDIUM_INPUT.to_string()));
        })
    });

    c.bench_function("Sliding Window - Medium", |b| {
        b.iter(|| {
            let _ = length_of_longest_substring_sliding_window(black_box(MEDIUM_INPUT.to_string()));
        })
    });
...
fn bench_large(c: &mut Criterion) {
    ...
    c.bench_function("Sliding Window - Large", |b| {
        b.iter(|| {
            let _ = length_of_longest_substring_sliding_window(black_box(large_input.clone()));
        })
    });
    ...
    c.bench_function("Linked HashMap Sliding Window - Large", |b| {
        b.iter(|| {
            let _ = length_of_longest_substring_linked_hash_map(black_box(large_input.clone()));
        })
    });
}
